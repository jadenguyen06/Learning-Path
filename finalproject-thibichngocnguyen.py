...
class Insights:
    ...
    def remove_stopwords(self):
        if self.stopwords:
            self.data['review'] = self.data['review'].apply(lambda x: ' '.join(word for word in x.split() if word not in self.stopwords))
        return self.data
    ...
    if 'parsed_url' in st.session_state and 'num_reviews' in st.session_state:
        ...
        if st.sidebar.checkbox("Sentiment Analysis and WordCloud"):
            ...
            if df is not None:
                ...
                user_stopwords = st.sidebar.text_area("Enter stopwords (one per line)", "")
                if user_stopwords:
                    wordcloud_generator.set_stopwords(user_stopwords)
                    wordcloud_generator.remove_stopwords()
                    st.header("Updated Word Clouds")
                    st.caption('Updated word clouds after removing stopwords')
                    wordcloud_generator.word_clouds_by_sentiment()
                    ...
                    st.info("Enter stopwords to update the word cloud.")
